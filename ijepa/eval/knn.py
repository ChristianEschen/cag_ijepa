...
from ijepa.eval.loaders import make_data_loader, SamplerType
from ijepa.eval.utils import ModelWithNormalize, evaluate, extract_features
# from torchmetrics import Metric, MetricCollection
# from torchmetrics.classification import MulticlassAccuracy
# from torchmetrics.classification.f_beta import MulticlassF1Score
from ijepa.eval.torchmetric import Metric, MetricCollection, MulticlassAccuracy, MulticlassF1Score
from ijepa.datasets.cagdataset import CAGDataset
...
class KnnModule(torch.nn.Module):
    ...
    def compute_neighbors(self, features_rank):
        for rank in range(self.global_size):
            # reduce features_rank by the mean across the first dimension
            if rank == 0:
...
def build_topk_accuracy_metric(average_type: AccuracyAveraging, num_classes: int, ks: tuple = (1, 5)):
    metrics1: Dict[str, Metric] = {
        f"top-{k}": MulticlassAccuracy(top_k=k, num_classes=int(num_classes), average=average_type.value) for k in ks
    }
    metrics2: Dict[str, Metric] = {
        f"F1-score": MulticlassF1Score(num_classes=int(num_classes), average='macro')
    }
    metrics = {**metrics1, **metrics2}
    return MetricCollection(metrics)

def build_f1_metric(num_classes: int, ks: tuple = (1)):
...
def eval_knn(
   ...
   # model_cop = copy.deepcopy(model) 
    ...
    rankme = calc_rankme(train_features)
    ...
    metric_collection = build_topk_accuracy_metric(accuracy_averaging, num_classes=num_classes, ks=(1,3))
   # metric_collection = build_f1_metric(num_classes=num_classes, ks=(1))
    ...
    #
    # add rankme to results_dict
    results_dict[('full', k)]["rankme"] = rankme
    return results_dict
...
def eval_knn_with_model(
    ...
    train_dataset= CAGDataset(args, transform, 'train_eval')
    val_dataset= CAGDataset(args, transform, 'val_eval')
    ...
    if distributed.is_main_process():
        for knn_ in results_dict_knn.keys():
           ...
           # top5 = results_dict_knn[knn_]["top-3"].item() * 100.0
            f1 = results_dict_knn[knn_]["F1-score"].item()# * 100.0
            rankme = results_dict_knn[knn_]["rankme"].item()# * 100.0
            results_dict[f"{knn_}_Top 1"] = top1
            #results_dict[f"{knn_}_Top 3"] = top5
            results_dict[f"{knn_}_F1-score"] = f1
            results_dict[f"{knn_}_rankme"] = rankme
            logger.info(f"{knn_} classifier result: Top1: {top1:.2f} F1-score: {f1:.2f}: RankMe: {rankme:.2f}")
    # write t
